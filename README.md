...
final id4 = letter().then([letter(), digit()].toChoiceParser().star());  // (4): Parser<(String, List<String>)>
```

> [!NOTE]
> The inferred type of the 4 parsers is not equivalent: Due to [github.com/dart-lang/language/issues/1557](https://github.com/dart-lang/language/issues/1557) the inferred type of sequence and choice parsers created with operators (0) or chained function calls (1) is `Parser<dynamic>`. The parser built from lists (2) provides the most generic type, `List<Object>` in this example. The variations (3) and (4) do not lose type information and produce a [record](https://dart.dev/language/records) (tuple) with two typed elements `String` and `List<String>`.
