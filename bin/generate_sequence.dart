...
Future<void> generateImplementation(int index) async {
  ...
  out.writeln(
    'Parser<(${resultTypes.join(', ')})> '
    'seq$index<${resultTypes.join(', ')}>('
    '${[for (var i = 0; i < index; i++) 'Parser<${resultTypes[i]}> ${parserNames[i]}'].join(', ')}'
    ') => ',
  );
  out.writeln(
  ...
  out.writeln(
    '/// Converts a [Record] of $index positional parsers to a '
    '[Parser] that runs the',
  );
  out.writeln(
    '/// parsers in sequence and returns a [Record] with $index '
  ...
  out.writeln(
    '/// A parser that consumes a sequence of $index parsers and '
    'returns a [Record] with ',
  );
  out.writeln('/// $index positional parse results.');
  out.writeln(
    'class SequenceParser$index<${resultTypes.join(', ')}> '
  ...
  for (var i = 0; i < index; i++) {
    out.writeln(
      'final ${resultNames[i]} = ${parserNames[i]}'
      '.parseOn(${i == 0 ? 'context' : resultNames[i - 1]});',
    );
    out.writeln(
      'if (${resultNames[i]} is Failure) '
      'return ${resultNames[i]};',
    );
  }
  out.writeln(
    'return ${resultNames[index - 1]}.success('
  ...
  out.writeln('@override');
  out.writeln(
    'SequenceParser$index<${resultTypes.join(', ')}> copy() => '
    'SequenceParser$index<${resultTypes.join(', ')}>'
    '(${parserNames.join(', ')});',
  );
  out.writeln('}');
  ...
  out.writeln(
    '/// Converts this [Record] with $index positional values to '
    'a new type [R] using',
  );
  out.writeln('/// the provided [callback] with $index positional arguments.');
  out.writeln('@preferInline');
  out.writeln(
    'R map<R>(R Function(${valueTypes.join(', ')}) callback) => '
  ...
  out.writeln(
    '/// Extension on a [Parser] producing a [Record] of $index '
    'positional values.',
  );
  out.writeln(
    'extension RecordParserExtension$index<${valueTypes.join(', ')}>'
