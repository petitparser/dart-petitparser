...
void main() {
  ...
  group('cut / noCut', () {
    final time1 = seq7(char('1').optional(), digit(), char(':'), cut(), digit(),
        digit(), [string('am'), string('pm')].toChoiceParser());
    final time2 = seq6(
        pattern('12').optional(), digit(), char(':'), cut(), digit(), digit());
    test('basic', () {
      expect(time1, isParseSuccess('12:30pm'));
      expect(time2, isParseSuccess('17:45'));
    });
    test('choice', () {
      final parser = [time1, time2].toChoiceParser();
      expect(parser, isParseSuccess('12:30pm'));
      expect(parser,
          isParseFailure('17:45', position: 5, message: '"pm" expected'));
    });
    test('choice without cut', () {
      final parser = [time1.noCut(), time2].toChoiceParser();
      expect(parser, isParseSuccess('12:30pm'));
      expect(parser, isParseSuccess('17:45'));
    });
  });
}
