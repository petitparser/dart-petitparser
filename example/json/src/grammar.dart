...
class JsonGrammarDefinition extends GrammarDefinition {
  ...
  array() => ref(token, char('[')) & ref(elements).optional() & ref(token, char(']'));
  ...
  object() => ref(token, char('{')) & ref(members).optional() & ref(token, char('}'));
  pair() => ref(stringToken) & ref(token, char(':')) & ref(value);
  value() =>
      ref(stringToken) | ref(numberToken) | ref(object) | ref(array) | ref(trueToken) | ref(
          falseToken) | ref(nullToken);
  ...
  characterPrimitive() => ref(characterNormal) | ref(characterEscape) | ref(characterUnicode);
  ...
  numberPrimitive() =>
      char('-').optional() & char('0').or(digit().plus()) & char('.')
          .seq(digit().plus())
          .optional() & pattern('eE').seq(pattern('-+').optional()).seq(digit().plus()).optional();
