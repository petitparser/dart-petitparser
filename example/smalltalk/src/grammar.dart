...
class SmalltalkGrammarDefinition extends GrammarDefinition {
  ...
  array() => ref(token, '{')
      ...
      .seq(ref(token, '}'));
  arrayItem() =>
      ref(literal).or(ref(symbolLiteralArray)).or(ref(arrayLiteralArray)).or(ref(byteLiteralArray));
  ...
  primary() => ref(literal).or(ref(variable)).or(ref(block)).or(ref(parens)).or(ref(array));
  ...
  statements() => ref(expression)
      .seq(ref(periodToken).plus().seq(ref(statements)).or(ref(periodToken).star()))
